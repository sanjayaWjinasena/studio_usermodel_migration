"""Pre-migrate v0.45: create 44 res.users x_studio_ columns via ALTER TABLE.

Runs during Apps->Upgrade BEFORE Odoo touches the model registry.
v0.46 will add the matching field declarations to res_users.py — columns
...
def migrate(cr, version):
    ...
    for name, sqltype in cols:
        try:
            ...
            _logger.warning("pre-migrate: failed res_users.%s (%s): %s", name, sqltype, e)
    _logger.info("pre-migrate v0.45: processed %d res.users columns (IF NOT EXISTS)", added)
