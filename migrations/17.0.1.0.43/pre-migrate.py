"""Pre-migrate: create 47 res.users x_studio_ columns via ALTER TABLE.

Runs BEFORE Odoo touches the model registry. When the Python class loads
its fields.X(...) declarations, columns already exist -> no _auto_init race.

Prior attempts (v0.38, v0.41) crashed because registry setup failed OR
_auto_init was skipped after a partial commit. Pre-migrate sidesteps both.
...
def migrate(cr, version):
    ...
    for name, sqltype in cols:
        try:
            ...
            # Log-and-continue: never block the migration on a single column
            import logging
            logging.getLogger(__name__).warning(
                "pre-migrate: failed to add res_users.%s (%s): %s", name, sqltype, e
            )
